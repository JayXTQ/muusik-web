...
export const actions: Actions = {
	default: async ({ fetch, locals, request }) => {
		...
		const guildoptions: Record<string, string> = {};
		let guild = false;
		...
		if (guild) {
			const res = await fetch(
				...
				}/get-owner?user=${encodeURIComponent(session.user.user_metadata.provider_id)}`
			);
			...
			if (guildoptions.api) {
				const res = await fetch(
					`${guildoptions.api}/get-owner?user=${encodeURIComponent(
			...
			const { error } = await supabase
				.from('guilds')
				.upsert({ id: data_.guild, settings: guildoptions }, { onConflict: 'id' });
...
export const load: PageServerLoad = async ({ fetch, locals }) => {
	...
	const res = await fetch(
		`${dev ? '//localhost:8000' : api}/get-owner?user=${encodeURIComponent(
			session?.user.user_metadata.provider_id
		)}`
	);
	const data_ = (await res.json()) as
